...
### Platform Awareness (Onyx Driver)

This project targets **web, Android, and iOS**. Every feature must work across all three.

| Need | Tool |
|---|---|
| Check environment (dev/prod) + platform | `AppMode` from `core/config/mode/app_mode.dart` — flags like `devMobile`, `prodWeb`, `devDebugMobile`, `devDebugWeb` |
| Get device/OS/version info | `getIt<AppDeviceInfo>()` (abstract interface from `core/utils/app_device_info/app_device_info.dart`) |
| Platform dispatch | `CurrentPlatform` from `core/config/platform/platform.dart` — `isMobile`, `isDesktop`, `isWeb`, `handle<T>()` for branching |

**Never assume mobile-only APIs** (e.g., `dart:io` Platform, GPS, local file system) — always guard with `AppMode` or `CurrentPlatform`. For testing patterns, see `test/core/config/platform/platform_test.dart`.

* **BLoC Context Reading:** Always access BLoCs/Cubits via typed `BuildContext` extension getters (e.g., `context.sidebarCubit` from `core/extension/bloc_reader.dart`). Store in a local variable (`final cubit = context.sidebarCubit;`) at top of `build()` to eliminate redundant $O(N)$ widget tree lookups.
...
* **Rule & Skill Synchronization:** Whenever updating or adding any AI rule, skill, or guideline, ALWAYS sync and update all AI configuration files (`.agent/rules/*.md`, `AGENTS.md`, `CLAUDE.md`, `.cursorrules`, `.github/copilot-instructions.md`, `GEMINI.md`, `.instructions.md`, `docs/ai/ai_rules.md`) across all three repositories (`Onyx_Onyxboard`, `onyx_todo`, `Onyx`).
